...
**Cruz, A.** "A Sensitivity Approach to Measurement Error Propagation."
...
<blockquote class="blockquote">
<p>
What is the effect of measurement error on downstream estimates? Answers to this question usually rely on strong assumptions, expensive simulations, or heavily-parameterized joint models, and the issue is often ignored in practice. In this paper, I propose a sensitivity approach that quickly bounds the maximum possible impact of measurement error on a downstream estimate, given only pointwise uncertainty estimates. The latter are available for a variety of social science variables, including those estimated from measurement models. I introduce a new sensitivity value, the m-value, that quantifies the amount of measurement error needed to invalidate a downstream estimate in an adversarial, worst-case scenario. Interpreting this quantity requires the judicious use of domain knowledge, which I illustrate with two empirical applications drawn from V-Dem scores and congressional ideal points. In line with other sensitivity tools, the m-value gives researchers a way to benchmark the plausibility of an inference threat, in this case stemming from measurement error. An open-source package will provide accessible R and Python interfaces to the proposed method.
</p>
</blockquote>
</details>
...
**Cruz, A.**, Gerring, J., & Knutsen, C.H. "Synthesizing Research through Total Model Space: The Case of Democracy and Growth."
